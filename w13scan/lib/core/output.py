...
from w13scan.lib.core.common import dataToStdout, md5
from w13scan.lib.core.data import KB, path, conf
...
class ResultObject(object):
    ...
    def add_detail(self, name: str, request: str, response: str, msg: str, param: str, value: str, position: str):
        ...
        elif urlparse(self.url).scheme == 'https':
            port = 443
        else:
            port = 80
        try:
            AnyX.objects.create(url=self.url, headers=request, type=self.type, msg=msg.encode('utf-8').decode('utf-8'), param=param, value=value, method=position, path=self.path,
                            result=self.result.encode('utf-8').decode('utf-8'),creatime=self.createtime, host=netloc, port=port)
